...
if ( ! class_exists( 'WP_Bootstrap_Navwalker' ) ) {
	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			$classes = array( 'dropdown-menu rounded-0 bg-transparent text-center' );
			/**
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( $args->has_children && 0 === $depth && $args->depth > 1 ) {
				...
				if ( $depth > 0 ) {
					$atts['class'] = 'dropdown-item text-white-50';
				} else {
					$atts['class'] = 'nav-link';
				}
			}
		...
		public static function fallback( $args ) {
			if ( current_user_can( 'edit_theme_options' ) ) {
				...
				$fallback_output .= '>';
				$fallback_output .= '<li><a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '" title="' . esc_attr__( 'Add a menu', 'clubfood' ) . '" class="nav-link">' . esc_html__( 'Add a menu', 'clubfood' ) . '</a></li>';
				$fallback_output .= '</ul>';
				if ( $container ) {
					$fallback_output .= '</' . esc_attr( $container ) . '>';
				}
